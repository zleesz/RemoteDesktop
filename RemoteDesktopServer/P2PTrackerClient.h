...
{
public:
	virtual void OnServerConnectSuccess(unsigned short tcp_port) = 0;
	virtual void OnServerConnectFailed() = 0;
	virtual void OnServerDisconnect() = 0;
	virtual bool OnPeerConnecting(PeerInfo& peer) = 0;
};
...
private:
	...
	void OnServerConnectSuccess(const char* buf);
	void OnServerConnectFailed();
	void OnServerPingSuccess();
	void OnServerPingFailed();
	void OnServerOfflineFinish(bool result);
	void OnPeerConnecting(PeerInfo& peer);
...
private:
	...
	P2PTRACKER_STATE			m_nTrackerState;
